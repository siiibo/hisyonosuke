...
export const setColors = (sheet: GoogleAppsScript.Spreadsheet.Sheet, colorConfigs: ColorConfig[]) => {
  colorConfigs.forEach(config => {
    ...
    sheet.createTextFinder(targetTextRegExp).useRegularExpression(true).findAll().forEach(range => {
...
export const setContents = (sheet: GoogleAppsScript.Spreadsheet.Sheet, contents: AttendanceHistory[]): GoogleAppsScript.Spreadsheet.Range => {
  return sheet.getRange(2, 1, contents.length, HEADER.length).setValues(contents.map(attendance => {
    return [attendance.dateString, attendance.userEmail, attendance.message, attendance.reactionFromBot, attendance.isRemote]
  }));
}
